...
It is **important** to note that the device tested had *Dialog firmware (version 1.23)* which was observed to block test user access if the device was in user mode, based on the file present in device. Given that most Sri Lankan operators' firmware is not updated and assuming every TOZED device has the same `tozed_param - config ver. S12U-SC001-v3.26UP`  file, there is a chance that the generated passwords could work on other devices.
